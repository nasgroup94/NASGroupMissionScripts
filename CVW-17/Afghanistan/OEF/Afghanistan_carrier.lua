...
CLIMAX = AIRBOSS:New("CVN-65", "CVN-65")

-- function CLIMAX:OnAfterStart(From, Event, To)
...
-- function CLIMAX:OnAfterPassingWaypoint(From, Event, To, n)
--     -- Recording waypoint to be used in the persistence script
--     -- PassingWaypoint(self.carrier:GetName(), n)
-- end

CLIMAX:SetFunkManOn(10042, "127.0.0.1")
-- CLIMAX:SetDebugModeON()
CLIMAX:SetMenuRecovery(60, 27, false, 0) --Curcuit changed to prevent boat from circling
CLIMAX:SetAutoSave(TRAPSHEETLOCATION)
CLIMAX:SetTrapSheet(TRAPSHEETLOCATION)
CLIMAX:Load()
CLIMAX:SetTACAN(65, "X", "CLM")
CLIMAX:SetICLS(5, "CLM")
CLIMAX:SetLSORadio(265)
CLIMAX:SetMarshalRadio(264)
CLIMAX:SetPatrolAdInfinitum()
CLIMAX:SetAirbossNiceGuy()
CLIMAX:SetDefaultPlayerSkill(AIRBOSS.Difficulty.NORMAL)
CLIMAX:SetMaxSectionSize(4)
CLIMAX:SetMPWireCorrection(12)
CLIMAX:SetRadioRelayLSO("CVN65_LSORELAY")
CLIMAX:SetRadioRelayMarshal("CVN65_MARSHALRELAY")
CLIMAX:SetSoundfilesFolder(AIRBOSSBASESOUNDFOLDER)
CLIMAX:SetVoiceOversLSOByRaynor(AIRBOSSLSORAYNOR)
CLIMAX:SetVoiceOversMarshalByGabriella(AIRBOSSMARSHALGABRIELLA)
CLIMAX:SetDespawnOnEngineShutdown()
CLIMAX:SetRecoveryTanker(RecoveryTanker)
CLIMAX:SetMenuSingleCarrier()
CLIMAX:SetHandleAIOFF()
CLIMAX:SetIntoWindLegacy()

CLIMAX.trapsheet = false
-- CLIMAX:SetDebugModeON()
-- CLIMAX.Debug = True

-- local CarrierExcludeSet=SET_GROUP:New():FilterPrefixes("Arco"):FilterStart()
-- CLIMAX:SetExcludeAI(CarrierExcludeSet)

function CLIMAX:OnAfterRecoveryStart(Event, From, To, Case, Offset)
    env.info(string.format("Starting Recovery Case %d ops.", Case))
end

-- Start airboss class.
CLIMAX:Start()
CLIMAX:AddRecoveryWindow("8:10","12:00",1,0,true,27,false)
...
-- HeliAirbossMenus = AIRBOSS_HELI:New({ CLIMAX, Tarawa })
