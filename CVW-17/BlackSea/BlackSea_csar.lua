...
BASE:I("BlackSea_csar.lua | Loading...")
...
local rescueMinTimeBetweenHeightCalls = 8
local rescueSmokeColor = 3 -- orange
local rescueHoverTime = 30
local mashPrefixes = {"MASH", "Mash"}
local csarPrefixes = {"HSM", "Whiplash", "Vertrep", "Pagan", "Huey", "RESCUEHELO", "HIP", "Blackhawk", "Blackshark", "Hind"}
...
local SRSGoogleCredsFile     = "c:\\hypeman\\api\\cvw7-tracking-11c8a6927776.json"
...
googleTTSSoundPath = "C:/hypeman/sound/csar/"
googleTTSCSARMsgs = {
    Copilot = {
...
function CSAR:PickMessagesForRescueUnit(woundedgroupname, rescueType, overWater)
    ...
    self:T({woundedgroupname = woundedgroupname, rescueType = rescueType, overWater = overWater})
...
function CSAR:messageQueueAddMsg(messageTime, groupName, functionToCall, ...)
    ...
    self:T({messageTime = messageTime, groupName = groupName, functionToCall = functionToCall, arg = arg, messageQueueTimer = self.MessageQueueTimer})
...
CSAR_PILOT.min_time_between_height_calls = rescueMinTimeBetweenHeightCalls
...
function CSAR_PILOT:OnAfterApproach(From, Event, To, Heliname, Woundedgroupname)
    ...
    if distBetween < 20000 and
        ...
        if distBetween <= rescueHoverDistance * 2 then
            ...
            -- We can control how ofter we call out heights by setting the min_time_between_height_calls property.
            if timer.getTime() >= self.RescueUnitsList[Woundedgroupname].HeightCallTimestamp + self.min_time_between_height_calls then
                ...
                self:messageQueueAddMsg(heightSoundFile:GetDuration(),
                                        ...
                                        rescueSRSFile,
                                        heightSoundFile)
                                        self.RescueUnitsList[Woundedgroupname].HeightCallTimestamp = timer.getTime()
            end
        end
...
function CSAR_PILOT:OnAfterPilotDown(From, Event, To, Group, Frequency, Leadername, CoordinatesText)
    ...
    self.RescueUnitsList[groupName] = {
        ...
        HeightCallTimestamp = timer.getTime()
    }
    ...
    dcsbot.sendBotMessage(string.format("CSAR: Pilot down | Coord: %s | Freq: %.2f", CoordinatesText, Frequency))
...
CSAR_SHIP.min_time_between_height_calls = rescueMinTimeBetweenHeightCalls
...
function CSAR_SHIP:OnAfterPilotDown(From, Event, To, Group, Frequency, Leadername, CoordinatesText)
    ...
    self.RescueUnitsList[groupName].CallFlags.NS_Mayday.FLAG = true
     ...
     -- Send message to Discord
     dcsbot.sendBotMessage(string.format("CSAR: Ship in distress | Coord: %s | Freq: %.2f", CoordinatesText, Frequency))
...
function CSAR_CLIENT_SET:OnEventPlayerEnterAircraft(EventData)
    ...
    -- Rescue Menus
    MENU_GROUP_COMMAND:New(group, "Spawn Downed Pilot ", helo_menu, PilotRescue, group, 1):Refresh()
    MENU_GROUP_COMMAND:New(group, "Spawn Sinking Ship", helo_menu, SailorRescue, group, 1):Refresh()
end

BASE:I("BlackSea_csar.lua | Loaded.")
