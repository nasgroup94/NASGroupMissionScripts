...
func (r *Radar) handleStarted() {
	log.Info().Msg("clearing all trackfiles due to mission.json (re)start")
	r.contacts.reset()

	log.Info().Msg("clearing pending FADED trackfiles due to mission.json (re)start")
	r.pendingFadesLock.Lock()
	defer r.pendingFadesLock.Unlock()
	r.pendingFades = make([]sim.Faded, 0)

	log.Info().Msg("clearing FADED trackfile history due to mission.json (re)start")
